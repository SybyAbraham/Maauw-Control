...
void drawMenuItems() {
	if (frame == 1 && selMenuItem == 1) {
		drawMenuItem(1, menuItem1, true);
		drawMenuItem(2, menuItem2, false);
		drawMenuItem(3, menuItem3, false);
	}
	else if (frame == 1 && selMenuItem == 2) {
		drawMenuItem(1, menuItem1, false);
		drawMenuItem(2, menuItem2, true);
		drawMenuItem(3, menuItem3, false);
	}
	else if (frame == 1 && selMenuItem == 3) {
		drawMenuItem(1, menuItem1, false);
		drawMenuItem(2, menuItem2, false);
		drawMenuItem(3, menuItem3, true);
	}
	else if (frame == 2 && selMenuItem == 2) {
		drawMenuItem(1, menuItem2, true);
		drawMenuItem(2, menuItem3, false);
		drawMenuItem(3, menuItem4, false);
	}
	else if (frame == 2 && selMenuItem == 3) {
		drawMenuItem(1, menuItem2, false);
		drawMenuItem(2, menuItem3, true);
		drawMenuItem(3, menuItem4, false);
	}
	else if (frame == 2 && selMenuItem == 4) {
		drawMenuItem(1, menuItem2, false);
		drawMenuItem(2, menuItem3, false);
		drawMenuItem(3, menuItem4, true);
	}
	else if (frame == 3 && selMenuItem == 3) {
		drawMenuItem(1, menuItem3, true);
		drawMenuItem(2, menuItem4, false);
		drawMenuItem(3, menuItem5, false);
	}
	else if (frame == 3 && selMenuItem == 4) {
		drawMenuItem(1, menuItem3, false);
		drawMenuItem(2, menuItem4, true);
		drawMenuItem(3, menuItem5, false);
	}
	else if (frame == 3 && selMenuItem == 5) {
		drawMenuItem(1, menuItem3, false);
		drawMenuItem(2, menuItem4, false);
		drawMenuItem(3, menuItem5, true);
	}
	else if (frame == 4 && selMenuItem == 4) {
		drawMenuItem(1, menuItem4, true);
		drawMenuItem(2, menuItem5, false);
		drawMenuItem(3, menuItem6, false);
	}
	else if (frame == 4 && selMenuItem == 5) {
		drawMenuItem(1, menuItem4, false);
		drawMenuItem(2, menuItem5, true);
		drawMenuItem(3, menuItem6, false);
	}
	else if (frame == 4 && selMenuItem == 6) {
		drawMenuItem(1, menuItem4, false);
		drawMenuItem(2, menuItem5, false);
		drawMenuItem(3, menuItem6, true);
	}
}
...
void drawMenu() {
	...
	if (page == 1) {    
		...
		convertToChar((String(tempC) + String("°C")), tempChar);
		u8g2.drawUTF8(centerLine(tempChar) + 7, 41, tempChar);
		...
		char statusTxt[] = "FIRING TO 10R";
		u8g2.drawUTF8(centerLine(statusTxt), 64, statusTxt);
	...
	if (page == 3) {
		...
		drawMenuItem(1, "Profile 1", true);
		drawMenuItem(2, "Profile 2", false);
	}

	if (page == 4) {
		if (profile[6].getExit() == false) {
			profile[6].setProfile(7);
			previousMillis = millis();
		}
		else {
			drawTitle("Profile 7");
			u8g2.setFont(u8g2_font_helvB10_tf);
			u8g2.drawUTF8(centerLine("Profile 7 Saved!"), 45, "Profile 7 Saved!");
			unsigned long currentMillis = millis();
			if (currentMillis - previousMillis >= 1000) {
				page = 1;
				profile[6].setExit(false);
			}
		}
	}
